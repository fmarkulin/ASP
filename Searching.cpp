...
template <typename T> class BinarySearchRecursive : public ISearch<T> {
 private:
   RetValSearch BSR(const T A[], const size_t left, const size_t right,
                    const T &item, int &attempts) {
      if (left > right)
         return RetValSearch{false, -1};
      int middle = (left + right) / 2;
      std::cout << "Recursively searching A[" << left << " : " << right
                << "]. Middle element: A[" << middle << "] = " << A[middle]
                << ". Attempts: " << ++attempts << ".\n";
      if (A[middle] == item)
         return {true, middle};
      else if (A[middle] > item)
         return BSR(A, left, middle - 1, item, attempts);
      else
         return BSR(A, middle + 1, right, item, attempts);
   }

 public:
   RetValSearch search(const T A[], const size_t n, const T &item) override {
      int attempts = 0;
      return BSR(A, 0, n - 1, item, attempts);
   }
};
      ...
      if (searchStrategy) {
         ...
         if (type == Linear) {
            ...
            if (!std::is_sorted(A, A + n)) {
               // raise exception, since binary search can
               // only be applied to sorted arrays
               throw std::invalid_argument(
                   "Error: Array not sorted! Binary search cannot be used.\n");
            }
            searchStrategy = new BinarySearch<T>();
         } else if (type == BinaryRecursive) {
            if (!std::is_sorted(A, A + n)) {
               // raise exception, since recursive binary search can
               // only be applied to sorted arrays
               throw std::invalid_argument(
                   "Error: Array not sorted! Recursive binary search cannot "
                   "be used.\n");
            }
            searchStrategy = new BinarySearchRecursive<T>();
         } else if (type == Jump) {
            if (!std::is_sorted(A, A + n)) {
               // raise exception, since binary search can
               // only be applied to sorted arrays
               throw std::invalid_argument(
                   "Error: Array not sorted! Jump search cannot be used.\n");
            }
...
int main(void) {
   ...
   try {
      while (true) {
         ...
         std::cout << "Enter search mode:\n"
                      "(1) Linear\n"
                      "(2) Linear-recursive\n"
                      "(3) Linear-recursive-sentinel\n"
                      "(4) Binary\n"
                      "(5) Binary recursive\n"
                      "(6) Jump/Block\n"
                      " > ";
